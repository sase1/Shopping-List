...
//
//             // If group code provided, check if it exists
//             if (groupIdToUse) {
...
//                 }
//
//                 // Add new user to the group's members array if not already included
...
//                 }
//             } else {
//                 // No group code → create a new group
//                 groupIdToUse = uuidv4();
//                 await setDoc(doc(db, 'groups', groupIdToUse), {
//                     name: `${user.email}'s group`,
//                     createdBy: user.uid,
//                     members: [user.uid] // include creator
//                 });
//             }
//
//             // Create user document with groupId
...
//                 groupIdToUse = uuidv4();
//                 await setDoc(doc(db, 'groups', groupIdToUse), {
//                     name: groupName.trim(), // use custom group name
//                     createdBy: user.uid,
//                     members: [user.uid],
//                 });
//             }
...
//                     <div>
//                         <label className="block text-gray-700 mb-1" htmlFor="groupCode">
//                             Group Code (enter if you have to join)
//                         </label>
//                         <input
//                             id="groupCode"
//                             type="text"
...
//                                 className="w-full border border-gray-300 px-4 py-2 rounded focus:outline-none focus:ring-2 focus:ring-sky-500 focus:border-sky-500 text-black"
//                                 required={!groupCode} // required only if not joining
//                             />
//                         </div>
//                     )}
//
...
export default function RegisterPage() {
    ...
    const [password, setPassword] = useState('');
    const [groupCode, setGroupCode] = useState('');
    const [groupName, setGroupName] = useState(''); // optional friendly name
    const [successMessage, setSuccessMessage] = useState('');
    const [errorMessage, setErrorMessage] = useState('');
    const router = useRouter();
    ...
    const handleRegister = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (groupIdToUse) {
                ...
                groupIdToUse = uuidv4(); // unique ID
                await setDoc(doc(db, 'groups', groupIdToUse), {
                    name: groupName.trim(), // friendly display name
                    createdBy: user.uid,
                    members: [user.uid],
                });
            }
